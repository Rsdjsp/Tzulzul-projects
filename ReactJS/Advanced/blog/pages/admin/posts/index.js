    ...
    <>
      <div className="flex justify-between my-10">
        ...
        <Link href="/admin/posts/create">
          <span className="bg-yellow-200 text-black p-2 hover:bg-yellow-300 rounded-md cursor-pointer">
            Crear publicación
          </span>
        </Link>
      </div>
      ...
      <section className="grid grid-cols-4 gap-5">
        {posts.map((post) => {
          return (
            <article className="bg-slate-700 p-5" key={post.id}>
              ...
              <Link href={`/admin/posts/${post.id}`}>
                <h3 className="text-3xl font-bold mb-3 cursor-pointer">
                  {post.title}
                </h3>
              </Link>
